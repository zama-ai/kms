...
impl KubernetesCmd {
    ...
    pub fn up(&self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for i in 1..kms_nb_pod {
            ...
            let kubernetes_logs_kms_core_init_load_env = Command::new("kubectl")
                .args([
                    "logs",
                    &format!("kms-core-{}", i),
                    "-c",
                    "kms-core-init-load-env",
                    "-n",
                    &std::env::var("NAMESPACE").unwrap(),
                ])
                .output()
            ...
            std::io::stdout()
                .write_all(&kubernetes_logs_kms_core_init_load_env.stdout)
                .expect("Failed to write logs to stdout");

            if !kubernetes_logs_kms_core_init_load_env.status.success() {
                let stderr =
                    String::from_utf8_lossy(&kubernetes_logs_kms_core_init_load_env.stderr);
