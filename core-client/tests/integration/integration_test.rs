...
///
/// # Note
/// This helper enables `ensure_default_prss=true` during server startup. It copies only signing
/// and client material so startup can create the default epoch before any FHE material refers to
/// it.
///
...
///
/// PRSS is ensured at server startup. Signing and client material come from
/// `test-material/default`; FHE material is generated by the test after the default epoch exists.
...
async fn setup_isolated_threshold_cli_test(
    ...
    // These CLI tests either generate their own FHE material after initializing an epoch or do
    // not use FHE material. Copying the pre-generated FHE fixture here would leave key shares
    // under DEFAULT_EPOCH_ID before EpochData exists, which is not a valid boot layout.
