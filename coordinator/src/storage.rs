use crate::{
    ...
    cryptography::central_kms::{compute_handle, SoftwareKmsKeys},
    kms::RequestId,
    rpc::{central_rpc::tonic_some_or_err, rpc_types::PrivDataType},
    some_or_err,
...
pub trait PublicStorageReader {
    ...
    fn data_exists(&self, url: &Url) -> anyhow::Result<bool>;

    /// Read some data from a given `url`.
    fn read_data<Ser: DeserializeOwned + serde::Serialize>(&self, url: &Url)
        -> anyhow::Result<Ser>;

    /// Compute an URL for some specific data given its `data_id` and of a given `data_type`.
    /// Depending on how the underlying functionality is realized one of these parameters might not be used.
    ...
    fn all_urls(&self, data_type: &str) -> anyhow::Result<HashMap<String, Url>>;
}

// Trait for KMS public storage reading and writing
// TODO rename away from public
pub trait PublicStorage: PublicStorageReader {
    fn store_data<T: serde::Serialize>(&mut self, data: T, url: &Url) -> bool;
    fn delete_data(&mut self, url: &Url) -> bool;
}

// Helper method for storing data based on a data type and request ID.
pub fn store_request_id<S: PublicStorage, Ser: serde::Serialize>(
    storage: &mut S,
    ...
    if !storage.store_data(data, &url) {
...
pub fn delete_request_id<S: PublicStorage>(
    storage: &mut S,
    request_id: &RequestId,
    data_type: &str,
) -> anyhow::Result<()> {
    let url = storage.compute_url(&request_id.to_string(), data_type)?;
    match storage.delete_data(&url) {
        true => Ok(()),
        false => Err(anyhow_error_and_log(format!(
...
pub fn read_request_id<S: PublicStorage, Ser: DeserializeOwned + serde::Serialize>(
    storage: &S,
    request_id: &RequestId,
    data_type: &str,
) -> anyhow::Result<Ser> {
    let url = storage.compute_url(&request_id.to_string(), data_type)?;
    storage.read_data(&url)
}

/// Helper method for reading all data of a specific type.
pub fn read_all_data<S: PublicStorage, Ser: DeserializeOwned + serde::Serialize>(
    storage: &S,
    data_type: &str,
) -> anyhow::Result<HashMap<RequestId, Ser>> {
    let url_map = storage.all_urls(data_type)?;
    let mut res = HashMap::with_capacity(url_map.len());
    for (data_ptr, url) in url_map.iter() {
        let data: Ser = storage.read_data(url)?;
...
impl PublicStorageReader for FileStorage {
    ...
    /// Return all elements stored of a specific type as a hashmap of the `data_ptr` as key and the `url` as value.
    fn all_urls(&self, data_type: &str) -> anyhow::Result<HashMap<String, Url>> {
...
impl PublicStorage for FileStorage {
    ...
    fn store_data<T: serde::Serialize>(&mut self, data: T, url: &Url) -> bool {
        ...
        match fs::create_dir_all(root_dir.clone()) {
...
impl RamStorage {
    ...
    pub fn from_existing_keys(keys: &SoftwareKmsKeys) -> anyhow::Result<Self> {
...
impl PublicStorageReader for RamStorage {
    ...
    fn read_data<Ser: DeserializeOwned + serde::Serialize>(
        &self,
        url: &Url,
    ) -> anyhow::Result<Ser> {
...
impl PublicStorage for RamStorage {
    fn store_data<T: serde::Serialize>(&mut self, data: T, url: &Url) -> bool {
        ...
        let data_id = match components.get(components.len() - 2) {
            Some(data_id) => data_id.to_string(),
            None => return false,
        };
        let data_type = match components.last() {
...
mod tests {
    ...
    fn threshold_dev_storage() {
        ...
        assert!(storage1.data_exists(&url).unwrap());
        let wrong = Url::from_file_path("/some/wrong/path/file.txt").unwrap();
        assert!(!storage1.data_exists(&wrong).unwrap());
