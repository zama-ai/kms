...
pub fn run_all_tests<M: TestedModule>(base_dir: &Path, pkg_version: &str) -> Vec<TestResult> {
    ...
    let meta: Vec<Testcase<<M as TestedModule>::Metadata>> =
        load_tests_metadata(&meta_file_path).unwrap_or_else(|e| {
            panic!(
                "Could not deserialize RON backwards compatibility metadata at {meta_file_path:?}: {e}"
            )
        });
