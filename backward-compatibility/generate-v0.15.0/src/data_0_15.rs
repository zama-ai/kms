...
use kms_0_15_0::cryptography::{
    encryption::{Encryption, PkeScheme, PkeSchemeType, UnifiedPrivateEncKey, UnifiedPublicEncKey},
...
use kms_0_15_0::util::key_setup::FhePublicKey;
use kms_0_15_0::vault::keychain::AppKeyBlob;
use kms_grpc_0_15_0::{
...
use threshold_execution_0_15_0::small_execution::prf::PrfKey;
use threshold_execution_0_15_0::tfhe_internals::public_keysets::FhePubKeySet;
use threshold_execution_0_15_0::{
...
use threshold_networking_0_15_0::tls::ReleasePCRValues;
use threshold_types_0_15_0::role::Role;
...
use backward_compatibility::{
    ...
    MlKem1024P384PrivateKeyTest, MlKem1024P384PublicKeyTest, NodeInfoTest, PRSSSetupTest,
    ...
    TestMetadataKmsGrpc, ThresholdFheKeysTest, TypedPlaintextTest, UnifiedPublicSigKeyTest,
    UnifiedSigncryptionKeyTest, UnifiedSigncryptionTest, UnifiedUnsigncryptionKeyTest,
    UserDecSignedPayloadTest, DISTRIBUTED_DECRYPTION_MODULE_NAME, KMS_GRPC_MODULE_NAME,
    KMS_MODULE_NAME,
};
...
/// fixtures. Using a constant instead of `SystemTime::now()` keeps the generated v0.15.0
...
// KMS test — for key generation metadata with the 15.0 format (including extra data).
...
// KMS test
const EPOCH_DATA_TEST: EpochDataTest = EpochDataTest {
    test_filename: Cow::Borrowed("epoch_data"),
    prss_setup_64: Cow::Borrowed("epoch_data_prss_setup_64"),
    prss_setup_128: Cow::Borrowed("epoch_data_prss_setup_128"),
    role_i: 3,
    amount: 13,
    threshold: 4,
    context_id: [8u8; 32],
...
impl KmsV0_15_0 {
    ...
    /// Twin of `gen_crs_metadata` for version 15.0 (with extra_data).
    ...
    /// `EpochData` was introduced in v0.15.0 as the new epoch-scoped storage type
    /// wrapping the (existing) `PRSSSetupCombined` together with the owning context ID.
    fn gen_epoch_data(dir: &PathBuf) -> TestMetadataKMS {
        let role = Role::indexed_from_one(EPOCH_DATA_TEST.role_i);
        let base_session_64 = get_networkless_base_session_for_parties(
            EPOCH_DATA_TEST.amount as usize,
            EPOCH_DATA_TEST.threshold,
            role,
        );
        let base_session_128 = get_networkless_base_session_for_parties(
            EPOCH_DATA_TEST.amount as usize,
        ...
        let epoch_data = EpochData {
            context_id: kms_grpc_0_15_0::ContextId::from_bytes(EPOCH_DATA_TEST.context_id),
            prss: PRSSSetupCombined {
                prss_setup_z64: prss_setup_z64.clone(),
                prss_setup_z128: prss_setup_z128.clone(),
                num_parties: EPOCH_DATA_TEST.amount,
                threshold: EPOCH_DATA_TEST.threshold,
            },
        };
        store_versioned_auxiliary!(
            &prss_setup_z64,
            dir,
            &EPOCH_DATA_TEST.test_filename,
            &EPOCH_DATA_TEST.prss_setup_64,
        );
        store_versioned_auxiliary!(
            &prss_setup_z128,
            dir,
            &EPOCH_DATA_TEST.test_filename,
            &EPOCH_DATA_TEST.prss_setup_128,
        );

        store_versioned_test!(&epoch_data, dir, &EPOCH_DATA_TEST.test_filename);
        TestMetadataKMS::EpochData(EPOCH_DATA_TEST)
    }
    ...
    fn gen_threshold_fhe_keys(dir: &PathBuf) -> TestMetadataKMS {
        ...
        let threshold_fhe_keys = ThresholdFheKeys::new(
            ...
            public_material,
            kms_0_15_0::engine::base::KeyGenMetadata::LegacyV0(info),
        );
...
impl DistributedDecryptionV0_15_0 {
...
impl KMSCoreVersion for V0_15_0 {
    const VERSION_NUMBER: &'static str = "0.15.0";
    ...
    fn gen_kms_data() -> Vec<TestMetadataKMS> {
        ...
        vec![
            ...
            KmsV0_15_0::gen_prss_setup_combined(&dir),
            ...
            KmsV0_15_0::gen_kms_fhe_key_handles(&dir),
    ...
    fn gen_threshold_fhe_data() -> Vec<TestMetadataDD> {
        ...
        vec![
            DistributedDecryptionV0_15_0::gen_prss_setup_rpoly_64(&dir),
            DistributedDecryptionV0_15_0::gen_prss_setup_rpoly_128(&dir),
            DistributedDecryptionV0_15_0::gen_prss_set_64(&dir),
            DistributedDecryptionV0_15_0::gen_prss_set_128(&dir),
            DistributedDecryptionV0_15_0::gen_share_64(&dir),
            DistributedDecryptionV0_15_0::gen_share_128(&dir),
            DistributedDecryptionV0_15_0::gen_prf_key(&dir),
            DistributedDecryptionV0_15_0::gen_release_pcr_values(&dir),
        ]
    }
    ...
    fn gen_kms_grpc_data() -> Vec<TestMetadataKmsGrpc> {
        ...
        vec![
            KmsGrpcV0_15_0::gen_signed_pub_data_handle_internal(&dir),
            KmsGrpcV0_15_0::gen_pub_data_type(&dir),
            KmsGrpcV0_15_0::gen_priv_data_type(&dir),
        ]
    }
}
