...
#
# EVM goldens are frozen by the digest test in core/service/tests/evm_path_byte_frozen.rs.
# Solana linker vectors are frozen by solana_frozen_constants.rs and the vector runner.
# Neither belongs on this path list: Solana is not shipped, and denying the EVM test file
# also froze Solana-kind cases that live next to the golden.
FROZEN_GLOBS=(
