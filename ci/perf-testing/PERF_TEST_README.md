...
| **Deployment type**                             | `threshold` → non-enclave `core-service-insecure`, path suffix `kms-ci`. `thresholdWithEnclave` → `core-service-enclave-insecure`, path suffix `kms-enclave-ci`.                             |
...
| **KMS Core image tag**                          | Used only when build is unchecked. Must be an existing insecure service or enclave tag that matches the deployment type.                                                                  |
| **KMS Core client image tag**                   | Used only when build is unchecked. Must be an existing `core-client-insecure` tag.                                                                                                             |
