...
// Synchronous (blocking, no-poll) user decryption endpoint. Benchmark variant.
pub const OP_USER_DECRYPT_DIRECT: &str = "user_decrypt_direct";
...
pub const TAG_OPERATION_TYPE: &str = "operation_type"; // operation-name label on the duration histogram (the counters expose the same value under TAG_OPERATION); part of the metrics' external contract
