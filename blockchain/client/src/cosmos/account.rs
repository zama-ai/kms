    ...
    pub fn new(prefix: &str, bytes: &[u8]) -> Result<Self, Error> {
        ...
        if !prefix.chars().all(|c| matches!(c, 'a'..='z' | '0'..='9')) {
            return Err(Error::InvalidAccount(format!(
                "expected prefix to be lowercase alphanumeric characters only - got {}",
                id
            )));
        }
        ...
        if matches!(bytes.len(), 1..=Self::MAX_LENGTH) {
            ...
            Err(Error::InvalidAccount(format!(
                "account ID should be at most {} bytes long, but was {} bytes long - id {}",
                Self::MAX_LENGTH,
                bytes.len(),
                id,
            )))
...
impl TryFrom<&AccountId> for tendermint::account::Id {
    ...
    fn try_from(id: &AccountId) -> Result<tendermint::account::Id, Self::Error> {
        ...
        match bytes.try_into() {
            Ok(bytes) => Ok(Id::new(bytes)),
            _ => Err(Error::InvalidAccount(format!(
                "invalid length for account ID: {} - id: {}",
                len,
                id.bech32.clone()
            ))),
        }
