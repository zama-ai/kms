...
if [ "$MODE" = "threshold" ]; then
  ...
  ASC_INST_DEBUG_TX_HASH=$(echo $PASSWORD | wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": true, "verify_proof_contract_addr": "dummy", "kms_core_conf": { "parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default"}, "allowed_addresses":{"allowed_to_gen": ["'"${CONN_ADDRESS}"'"], "allowed_to_response": ["'"${CONN_ADDRESS}"'"]} }' --label "asc" --from validator --output json --chain-id testing --node tcp://localhost:26657 -y --admin "${VALIDATOR_ADDRESS}" | jq -r '.txhash')

  sleep 6

  echo "Instantiating centralized ASC Ethermint"
  ASC_INST_ETHERMINT_TX_HASH=$(echo $PASSWORD | wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": false, "verify_proof_contract_addr": "'${IPSC_ETHERMINT_ADDRESS}'", "kms_core_conf": { "parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default"}, "allowed_addresses":{"allowed_to_gen": ["'"${CONN_ADDRESS}"'"], "allowed_to_response": ["'"${CONN_ADDRESS}"'"]} }' --label "asc" --from validator --output json --chain-id testing --node tcp://localhost:26657 -y --admin "${VALIDATOR_ADDRESS}" | jq -r '.txhash')

  sleep 6

  echo "Instantiating centralized ASC Ethereum"
  ASC_INST_ETHEREUM_TX_HASH=$(echo $PASSWORD | wasmd tx wasm instantiate "${ASC_CODE_ID}" '{"debug_proof": false, "verify_proof_contract_addr": "'${IPSC_ETHEREUM_ADDRESS}'", "kms_core_conf": { "parties":[{"party_id": "01", "address": ""}], "response_count_for_majority_vote": 1, "response_count_for_reconstruction": 1, "degree_for_reconstruction": 0, "param_choice": "default"}, "allowed_addresses":{"allowed_to_gen": ["'"${CONN_ADDRESS}"'"], "allowed_to_response": ["'"${CONN_ADDRESS}"'"]} }' --label "asc" --from validator --output json --chain-id testing --node tcp://localhost:26657 -y --admin "${VALIDATOR_ADDRESS}" | jq -r '.txhash')
