...
PUBLIC_KEY_FOLDER_NAME="PublicKey"
SERVER_KEY_FOLDER_NAME="ServerKey"

# find /app/kms/core/service/keys/ -path "/app/kms/core/service/keys/PUB*/PublicKey/*" -type f | while read -r file; do
# Push public keys to minio
find ./keys/ -path "*/PUB*/${PUBLIC_KEY_FOLDER_NAME}/*" -type f | while read -r file; do

  FOLDER_NAME=$(echo "$file" | sed -n 's|.*\(PUB[^/]*\)/PublicKey/.*|\1|p')
  ...
  resource="/${BUCKET_NAME}/${FOLDER_NAME}/${PUBLIC_KEY_FOLDER_NAME}/${FILE_NAME}"
...
# Push server key
find ./keys/ -path "*/PUB*/${SERVER_KEY_FOLDER_NAME}/*" -type f | while read -r file; do

  FOLDER_NAME=$(echo "$file" | sed -n 's|.*\(PUB[^/]*\)/ServerKey/.*|\1|p')
  KEY_NAME=$(basename "$file")
  FILE_NAME=$(basename "$file")
  FILE_PATH=$(realpath "$file")
  ...
  # Extract the host from the endpoint
  HOST=$(echo "${ENDPOINT}" | sed -e 's|^[^/]*//||' -e 's|/.*$||')
  resource="/${BUCKET_NAME}/${FOLDER_NAME}/${SERVER_KEY_FOLDER_NAME}/${FILE_NAME}"
  content_type="application/octet-stream"
  date=$(TZ=UTC date -R)
  _signature="PUT\n\n${content_type}\n${date}\n${resource}"
  echo "'$_signature'"
