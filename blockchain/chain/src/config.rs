...
/// // Create our server, binding it to TCP port 36658 on localhost and
/// // supplying it with our key/value store application
/// let server = ServerBuilder::default().bind("127.0.0.1:36658", app).unwrap();
...
impl ConfigApp {
    /// Constructor.
    pub fn new() -> (Self, ConfigDriver) {
        let (cmd_tx, cmd_rx) = channel();
        (Self { cmd_tx }, ConfigDriver::new(cmd_rx))
    }
