...
start_tcp_proxy_out() {
    ...
    socat -T60 VSOCK-LISTEN:"$VSOCK_PORT",fork,reuseaddr,shut-down TCP:"$TCP_DST",nodelay &
}
...
start_tcp_proxy_in() {
    ...
    local PORT="$2"
    echo "start_proxies: starting parent-side $NAME proxy"
    socat -T60 TCP-LISTEN:"$PORT",fork,nodelay,reuseaddr,shut-down VSOCK-CONNECT:"$ENCLAVE_CID":"$PORT"
}
...
socat -T60 VSOCK-LISTEN:"$ENCLAVE_CONFIG_PORT",fork,reuseaddr,shut-down OPEN:"$KMS_SERVER_CONFIG_FILE",rdonly &
