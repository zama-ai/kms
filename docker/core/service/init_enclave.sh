...
UPSTREAM_NS=$(grep nameserver resolv.conf | cut -d ' ' -f 2)
...
# DNS proxy, using TCP (not UDP, as typical in DNS) is important because UDP is
# stateless, proxying UDP both ways would require two pairs of vsock proxies,
# not one as for TCP, but we still have to resolve names over UDP
log "starting enclave-side DNS proxy"
dnsproxy -v -u "tcp://$UPSTREAM_NS" |& logger &
sleep 2
log "enclave /etc/resolv.conf with dnsproxy:"
sed -i 's/nameserver.*$/nameserver 127.0.0.1/' /etc/resolv.conf |& logger
cat /etc/resolv.conf |& logger
