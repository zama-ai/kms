...
mod only_testing_features {
    ...
    use crate::vault::storage::{
        file::FileStorage, read_all_data_versioned, store_pk_at_request_id,
        store_text_at_request_id, store_versioned_at_request_id, Storage, StorageForBytes,
        StorageReader, StorageType,
    };
    ...
    use rcgen::{
        ...
        KeyUsagePurpose, PKCS_ECDSA_P256K1_SHA256,
    };
        ...
        PrivS: Storage,
    {
        ...
        let pub_fhe_map =
            HashMap::from([(*key_id, fhe_pub_keys_1), (*other_key_id, fhe_pub_keys_2)]);
        ...
        PrivS: StorageForBytes,
    {
        ...
        for (i, subject_str) in parties {
            ...
            let temp: HashMap<
                RequestId,
                crate::cryptography::internal_crypto_types::PrivateSigKey,
        ...
        PrivS: Storage,
    {
            ...
            .zip_eq(priv_storages.iter_mut().zip_eq(signing_keys.iter()))
        {
            ...
            let crs_info =
                compute_info(cur_sk, &DSEP_PUBDATA_CRS, &pp, &domain).unwrap_or_else(|e| {
            ...
            store_versioned_at_request_id::<PrivS, kms_grpc::rpc_types::SignedPubDataHandleInternal>(
            cur_priv,
            crs_handle,
            &crs_info,
            &PrivDataType::CrsInfo.to_string(),
...
#[cfg(feature = "testing")]
pub use self::only_testing_features::*;
