...
impl<T> MetaStore<T> {
    ...
    pub(crate) fn insert(
        ...
        let claim = Arc::new(());
        let permit = MetaStorePermit::new(*request_id, Arc::clone(&claim), self.reaper_tx.clone());
        self.storage
    ...
    pub(crate) fn get_successful_completed_request_ids(&self) -> Vec<RequestId> {
        self.complete_queue
            ...
            .filter_map(|id| match self.storage.get(id) {
                Some(StoredEntry::Done(Ok(_), _)) => Some(*id),
                Some(_) => None,
                None => {
...
mod tests {
    ...
    fn lock_entry_on_done_then_delete() {
        ...
        assert_eq!(store.get_successful_completed_count(), 1);
        let permit = store.lock_entry(&id).unwrap();
        ...
        assert_eq!(store.get_successful_completed_count(), 0);
    }
    ...
    fn new_from_map_seeds_completed_entries() {
        ...
        assert_eq!(store.get_successful_completed_count(), 2);
    ...
    async fn with_overwriting_claim_overwrites_existing() {
        ...
        assert_eq!(store.read().await.get_successful_completed_count(), 1);
