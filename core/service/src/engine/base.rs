...
pub(crate) enum SigningApproach {
    /// EIP-712 recoverable ECDSA signature over a 32-byte signing hash, for
    /// on-chain `ecrecover`.
    Eip712,
...
pub(crate) struct SchemeSigningJob {
    pub scheme: SigningSchemeType,
    pub approach: SigningApproach,
    pub message: Vec<u8>,
}
