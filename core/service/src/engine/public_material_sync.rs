...
//!
//! Every party in an MPC context publishes the same keysets and CRSes, so a
//! node whose own copy is missing or damaged can repair it from a peer's public
//! storage — the peer URLs are recorded in the
//! [`ContextInfo`](crate::engine::context::ContextInfo) entries of private
//! storage.
//!
//! Downloaded bytes are trusted if they hash, under the correct domain
//! separator, to a digest taken from this node's own, trusted, private metadata
//! ([`KeyGenMetadata`] / [`CrsGenMetadata`]). Only digest-validatable material
//! is ever fetched: current keyset metadata (`PublicKey`, `ServerKey`,
//! `CompressedXofKeySet`) and current CRS metadata. Legacy metadata carries no
//! digest, `DecompressionKey` has no private counterpart, and the node-specific
//! types (verification keys and addresses, CA certificates, recovery material)
//! exist per party.
...
/// The protocol [`split_url`] reports for a peer whose public storage is a directory on disk.
const FILE_PROTOCOL: &str = "file://";

/// Build an anonymous read-only client for one peer's public storage.
///
/// A node with an empty storage URL, or one whose storage is a local directory rather than an
/// S3 bucket fails. Only S3 buckets are supported in [NodeInfo].
    ...
    R: StorageReader,
{
    ...
    // Only S3-backed peers can be read from here. `file://` is the other protocol `split_url`
    // accepts, and it names a directory rather than a bucket, so it is rejected by name.
    anyhow::ensure!(
        protocol != FILE_PROTOCOL,
        "public storage URL {} is file-backed, which the peer sync does not support",
        node.public_storage_url
    );
...
///
/// Metadata for the same ID under several epochs is collapsed to the greatest
/// epoch since one public material may correspond to private materials from
/// many epochs (e.g., through many resharing).
///
/// Every needed entry that cannot be fetched and verified from any peer is an error.
    ...
    R: StorageReader,
{
    ...
    for item in work {
        ...
        let context = match context_cache.entry(*context_id) {
            ...
            Entry::Vacant(entry) => match read_context_at_id(private_storage, context_id).await {
...
mod tests {
    ...
    #[tokio::test]
    async fn fetcher_names_the_reason_a_file_url_peer_is_skipped() {
        let mut rng = AesRng::seed_from_u64(20);
        let id = RequestId::new_random(&mut rng);
        let nodes = vec![make_node(1, "file:///tmp/some-peer-material")];
        ...
        // The operator needs the real reason, not a complaint about an unparseable domain.
        assert!(err.to_string().contains("is file-backed"), "{err:#}");
    }
