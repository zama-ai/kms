...
// [StableUserDecryptionTestVector] (produced by [TestingUserDecryptionTranscript::to_stable_test_vector])
...
impl TestingUserDecryptionTranscript {
    /// Convert this in-memory transcript into a [StableUserDecryptionTestVector].
    ///
    /// `fhe_parameter` is the name (`"test"` / `"default"`) that, when passed to `new_client`,
    /// reconstructs the same [`DKGParams`](threshold_execution::tfhe_internals::parameters::DKGParams)
    /// that were used to produce this transcript. The encodings here are byte-identical to the
    /// stable WASM helpers, so the resulting JSON round-trips through the public API unchanged.
    pub(crate) fn to_stable_test_vector(
        &self,
        fhe_parameter: &str,
    ) -> anyhow::Result<StableUserDecryptionTestVector> {
        let request = self
            .request
            .as_ref()
        ...
        let enc_pk = {
            ...
            tfhe::safe_serialization::safe_serialize(
                &UnifiedPublicEncKey::MlKem512(self.eph_pk.clone().unwrap_ml_kem_512()),
                &mut buf,
...
// ([TestingUserDecryptionTranscript::to_stable_test_vector]) on the host. It is therefore only
