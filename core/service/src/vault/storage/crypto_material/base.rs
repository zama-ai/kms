...
use crate::{
    ...
    vault::{
        ...
        storage::{
            ...
            read_all_data_versioned, read_context_at_id,
        },
    },
};
use kms_grpc::{
    ...
    PrivS: StorageExt + Send + Sync + 'static,
{
    ...
    /// Stores a public/private pair (FHE keys or CRS/metadata), or a single entry, then optionally backs it up.
    ///
    /// Returns `Duplicate` if all requested entries exist. Otherwise it keeps existing entries
    /// and fills the missing half; the caller must ensure the halves belong together.
    /// Failed-write cleanup removes new entries, never ones that pre-existed or were skipped.
    /// Callers must serialize writes to the same entries through cleanup.
    ...
    pub(crate) async fn write_crs(
        ...
        .await;
        update_meta_store(res, crs_info, &meta_store, permit, op_metric_tag).await
    }
