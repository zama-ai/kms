...
pub fn calculate_max_num_bits(dkg_params: &DKGParams) -> usize {
    ...
    // Cache the params to avoid calling it twice
    let params = dkg_params;

    // Try to calculate max_messages, but fall back to default if it fails
    let max_messages = match max_num_messages(&params.compact_pk_enc_params(), DEFAULT_MAX_NUM_BITS)
    {
    ...
    if params.lwe_dimension().0 < max_messages.0 {
        tracing::warn!(
