...
pub mod tests {
    ...
    #[tokio::test]
    async fn test_store_data_does_not_overwrite_existing() {
        let mut storage = ram::RamStorage::new();
        let req_id = derive_request_id("overwrite_test").unwrap();
        let data_type = "TestType";
        let first = TestType { i: 100 };
        let second = TestType { i: 200 };

        // Store first value
        storage
            .store_data(&first, &req_id, data_type)
            .await
            .unwrap();

        // Attempt to store second value with same id/type
        storage
            .store_data(&second, &req_id, data_type)
            .await
            .unwrap();

        // Read back and ensure it's still the first value
        let retrieved: TestType = read_versioned_at_request_id(&storage, &req_id, data_type)
            .await
            .unwrap();
    ...
    #[tokio::test]
    async fn test_store_bytes_does_not_overwrite_existing() {
        let mut storage = ram::RamStorage::new();
        let req_id = derive_request_id("bytes_overwrite_test").unwrap();
        let data_type = "BytesType";
        let first = b"first".to_vec();
        let second = b"second".to_vec();

        // Store first bytes
        storage
            .store_bytes(&first, &req_id, data_type)
            .await
            .unwrap();

        // Attempt to store second bytes with same id/type
        storage
            .store_bytes(&second, &req_id, data_type)
            .await
            .unwrap();

        // Read back and ensure it's still the first bytes
        let retrieved = storage.load_bytes(&req_id, data_type).await.unwrap();
        assert_eq!(retrieved, first);
    }
}
