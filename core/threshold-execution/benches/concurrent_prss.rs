...
fn bench_ring<Z: ErrorCorrect + Invert + PRSSConversions>(c: &mut Criterion, ring: &str) {
    ...
    let _guard = rt.enter();
    let prss_setup = support::setup_prss::<Z>(&rt, 13, THRESHOLD);
    let mut group = c.benchmark_group(format!("concurrent_prss/{ring}/parties_13_threshold_4"));
    ...
    for sessions in [1_usize, 2, 5, 10] {
