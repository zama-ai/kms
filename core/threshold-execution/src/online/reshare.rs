...
impl<
    ...
    OpenProtocol: RobustOpen,
    BroadcastProtocol: Broadcast,
> ProtocolDescription for RealTwoSetsReshareAsSet1<TwoSetsSes, OpenProtocol, BroadcastProtocol>
{
    fn protocol_desc(depth: usize) -> String {
        let indent = Self::INDENT_STRING.repeat(depth);
        format!(
...
impl<
    ...
    RobustOpenProtocol: RobustOpen,
    BroadcastProtocol: Broadcast,
> Clone for RealTwoSetsReshareAsSet2<TwoSetsSes, OneSetSes, RobustOpenProtocol, BroadcastProtocol>
{
    fn clone(&self) -> Self {
...
mod tests {
    ...
    impl LateParties {
        fn is_late(self, role: &TwoSetsRole) -> bool {
            match self {
                ...
                LateParties::OneOfSet2 => *role == TwoSetsRole::Set2(Role::indexed_from_one(4)),
            }
        }
    }
    ...
    async fn test_reshare_two_sets_fails_on_late_set_2_without_advance() {
        ...
        for (role, result) in on_time {
            assert!(matches!(role, TwoSetsRole::Set1(_)));
            assert!(
                result.is_err(),
