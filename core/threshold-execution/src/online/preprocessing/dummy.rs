...
use crate::runtime::sessions::session_parameters::ParameterHandles;
use crate::runtime::sessions::session_parameters::SessionParameters;
...
impl ProtocolDescription for DummyPreprocessing {
    ...
    > Preprocessing<Z, S> for DummyPreprocessing
{
    async fn execute(
        &mut self,
        _session: &mut S,
        batch_sizes: crate::config::BatchParams,
    ...
    ResiduePoly<Z128, EXTENSION_DEGREE>: ErrorCorrect + Invert + Solve,
{
    ...
    /// Fill the masks directly from the [`crate::small_execution::prss::PRSSState`] available from [`SmallSession`]
    ...
    /// using [`crate::online::secret_distributions::SecretDistributions`]
        ...
        ($z:ty, $u:ty) => {
            paste! {
                ...
                fn [<test_threshold_dummy_share $z:lower>]() {
                    ...
                    let shares = DummyPreprocessing::share::<ResiduePolyF4<$z>>(
                        session.num_parties(),
                        session.threshold(),
                        msg,
                        session.rng(),
