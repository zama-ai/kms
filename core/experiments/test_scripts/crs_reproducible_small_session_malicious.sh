...
# `malicious_broadcast` party (4 parties, threshold 1, party 1 misbehaves).
...
# with one malicious_broadcast party. The CRS gen protocol is supposed to
...
# TODO: replace the placeholder zeros with the actual hashes after the
# first successful run; the script's hash check prints the produced hash
# on mismatch.
