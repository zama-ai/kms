    ...
    ResiduePoly<Z, EXTENSION_DEGREE>: ErrorCorrect,
{
    ...
    let packing_key_switching_key_shares = allocate_and_generate_lwe_packing_keyswitch_key(
        ...
        &private_key.post_packing_ks_key,
...
#[instrument(name="Gen Sns Compression Key", skip(glwe_secret_key_share_sns_as_lwe, private_key, mpc_encryption_rng, session, preprocessing), fields(sid = ?session.session_id(), my_role = ?session.my_role()))]
...
#[instrument(name="Gen compressed Sns Compression Key", skip(glwe_secret_key_share_sns_as_lwe, private_key, mpc_encryption_rng, session, preprocessing, seed), fields(sid = ?session.session_id(), my_role = ?session.my_role()))]
