...
`Unexpected private material`, `Unexpected data type in private storage`,
`Could not list private material`, `Default epoch is not initialized` (keys are staged but `init`
has not run yet), and `Default context is missing` (startup rewrites it from the peer list). Right
after a custodian recovery, split legacy `PrssSetup` entries are expected until the next start
migrates them.
