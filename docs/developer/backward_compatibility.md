...
4. Add to `Makefile`:
   - Set `DETERMINISTIC_BWC_VERSIONS` to the new version number (e.g. `0.16.0`) — it is the only version regenerated by `-all`. **Do not** add the new version to `FROZEN_BWC_VERSIONS`.
   - Move the version it supersedes (the previous deterministic version, e.g. `0.15.0`) **into** `FROZEN_BWC_VERSIONS`, so its released data is no longer regenerated.
