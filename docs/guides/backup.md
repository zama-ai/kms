...
| Party | What it does |
|---|---|
| **Custodian `B_j`** (`j = 1..n`) | Human-held, offline party. Owns a long-term signing key `sk^{S_j}` (ECDSA/secp256k1) and a post-quantum encryption key `sk^{E_j}` (MLKEM1024-P384, the composite of ML-KEM-1024 and P-384), both deterministically derived from a 24-word BIP39 seed phrase. A single draw `SHAKE256("MNEMONIC" ‖ entropy)` of 64 bytes gives both keys: the first 32 bytes are the seed of `sk^{E_j}`, and the last 32 bytes are the custodian's root signing seed, from which `sk^{S_j}` is derived. Neither half passes through an intervening PRNG, so none of the phrase's 256 bits are lost. Stores nothing online beyond its public-key published in the `CustodianSetupMessage`. Re-signcrypts its share of the backup key on request. |
...
sequenceDiagram
    ...
    Op->>Vault: RecoveryValidationMaterial (unencrypted)
    end
    ...
    Vault-->>Op: RecoveryValidationMaterial
