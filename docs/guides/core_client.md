...
- Each KMS core is configured under a separate `[[cores]]` section containing the following values:
  ...
  - `address`- The host name or IP addresses with port of the cores service interface. Example: `address = "localhost:50100"`.
  - `s3_endpoint` - The host name or IP address, and a port of the S3 endpoint that is used for public data (public keys, CRS, etc.). Example" `s3_endpoint = "http://localhost:9000/kms"`.
